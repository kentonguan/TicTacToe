...
fun TicTacToeBoardState.isHorizontallyComplete(lastMove: TicTacToeMove?): Boolean {
    val lastMoveOwnerOrdinal = lastMove?.owner?.ordinal ?: return false
...
fun TicTacToeBoardState.isLeftDiagonalComplete(lastMove: TicTacToeMove?): Boolean {
    val lastMoveOwnerOrdinal = lastMove?.owner?.ordinal ?: return false
...
fun TicTacToeBoardState.isRightDiagonalComplete(lastMove: TicTacToeMove?): Boolean {
    val lastMoveOwnerOrdinal = lastMove?.owner?.ordinal ?: return false
    for (coordinate in 0 until size) {
